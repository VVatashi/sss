...
using SysChaCha20Poly1305 = System.Security.Cryptography.ChaCha20Poly1305;

namespace SimpleShadowsocks.Protocol.Crypto;

internal sealed class ChaChaAeadCipherImpl : IAeadCipherImpl
{
    ...
    public ChaChaAeadCipherImpl(ReadOnlySpan<byte> key)
    {
        if (key.Length != 32)
        {
    ...
    public int Encrypt(ReadOnlySpan<byte> plaintext, ReadOnlySpan<byte> nonce, Span<byte> destination)
    {
        if (Volatile.Read(ref _systemSupported) == 1)
        {
            try
            {
                using var cipher = new SysChaCha20Poly1305(_key);
                cipher.Encrypt(
                    nonce,
                    plaintext,
    ...
    private static bool ProbeSystem()
    {
        try
        {
            ...
            using var aead = new SysChaCha20Poly1305(key);
