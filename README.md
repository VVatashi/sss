...
`Socks5Authentication` управляет входящей аутентификацией локального SOCKS5-сервера:

- `Enabled: false` оставляет текущее поведение без авторизации (`NO AUTH`).
- `Enabled: true` переводит сервер на `USERNAME/PASSWORD` (RFC 1929) и требует валидные `Username`/`Password` для каждого входящего подключения.
- `Username` и `Password` задаются в `appsettings.json`; CLI-аргументы для них не предусмотрены.
...
- `Tunnel Port`
- `Shared Key`
- `Traffic Routing Rules` (по умолчанию одно правило `* -> Tunnel`; правила проверяются сверху вниз, можно добавлять, удалять и менять местами)
- `Enable SOCKS5 Authentication` (опционально)
- `SOCKS5 Username` / `SOCKS5 Password` (появляются только если включена авторизация)
- `Cipher Algorithm`
...
Если Android VPN запущен с включённой `SOCKS5`-авторизацией, встроенный `hev-socks5-tunnel` использует те же `Username` / `Password` для подключения к локальному SOCKS5-серверу приложения.
...
При включённом `Socks5Authentication` в `appsettings.json` скрипт `run-full-tunnel.ps1` переносит `Username` и `Password` в сгенерированный `hev-socks5-tunnel.yml`, чтобы системный туннель подключался к локальному SOCKS5 с той же авторизацией.
...
- SOCKS5 auth: локальный сервер по умолчанию принимает `NO AUTH`, а при включении `Socks5Authentication` требует `USERNAME/PASSWORD`, пропускает `CONNECT` и `UDP ASSOCIATE` только после успешной аутентификации и закрывает сессию после failed auth.
