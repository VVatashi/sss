...
- сжатие `Deflate`, `Gzip`, `Brotli` для протоколов `v2` и `v3`;
...
- поддерживает протоколы `v1`, `v2` и `v3`;
...
- разные AEAD-алгоритмы и компрессия `v2`/`v3`;
...
- `VER` - версия кадра (`1`, `2` или `3`);
- `TYPE` - `Connect(1)`, `Data(2)`, `Close(3)`, `Ping(4)`, `Pong(5)`, `UdpAssociate(6)`, `UdpData(7)`, `HttpRequest(8)`, `HttpRequestEnd(9)`, `HttpResponse(10)`, `ReverseHttpRequest(11)`, `ReverseHttpRequestEnd(12)`, `ReverseHttpResponse(13)`;
- `FLAGS` в `v2`/`v3`:
...
- сервер принимает `v1`, `v2` и `v3`;
- клиент валидирует версию ответов;
- при нарушении monotonic `SEQUENCE` сессия закрывается;
- компрессия доступна в `v2` и `v3`;
- HTTP relay доступен только в `v3`.
